...
def ARy_T(k_ARc,XnYnZn,globals_par,AR_bounds,LogLc,xc,yc,zc,rhoc,ARgc,ARTc,T,Kernel_Grv,Kernel_Mag,dg_obs,dT_obs,bk_AR):
    ...
    if step ==92:
        ...
        for iar in np.arange(kAR):
            ...
            ARTp_2D = ARTc_2D.copy()
            ARTp_2D[iar+1,0] = cauchy_dist(ARTc_2D[iar+1,0],std_cauchy,AR_min,AR_max,ARTc_2D[iar+1,0])
            if np.isclose(ARTc_2D[iar+1,0] , ARTp_2D[iar+1,0])==1: continue
