...
def ARx_g(k_ARc,XnYnZn,globals_par,AR_bounds,LogLc,xc,yc,zc,rhoc,ARgc,ARTc,T,Kernel_Grv,Kernel_Mag,dg_obs,dT_obs,bk_AR):
    ...
    if step ==92:
        ...
        for iar in np.arange(kAR):
            ...
            ARgp_2D = ARgc_2D.copy()
            ARgp_2D[0,iar+1] = cauchy_dist(ARgc_2D[0,iar+1],std_cauchy,AR_min,AR_max,ARgc_2D[0,iar+1])
            if np.isclose(ARgc_2D[0,iar+1] , ARgp_2D[0,iar+1])==1: continue
