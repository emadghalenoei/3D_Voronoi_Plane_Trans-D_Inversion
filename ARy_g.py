...
def ARy_g(k_ARc,XnYnZn,globals_par,AR_bounds,LogLc,xc,yc,zc,rhoc,ARgc,ARTc,T,Kernel_Grv,Kernel_Mag,dg_obs,dT_obs,bk_AR):
    ...
    if step ==92:
        ...
        for iar in np.arange(kAR):
            ...
            ARgp_2D = ARgc_2D.copy()
            ARgp_2D[iar+1,0] = cauchy_dist(ARgc_2D[iar+1,0],std_cauchy,AR_min,AR_max,ARgc_2D[iar+1,0])
            if np.isclose(ARgc_2D[iar+1,0] , ARgp_2D[iar+1,0])==1: continue
